...
submission_fake = submission_logreg %>%
  left_join(all_games %>% filter(set=='train') %>% dplyr::select(ID, outcome=Pred) %>% distinct() %>% mutate(is_known = T), by='ID', fill=NA) %>%
  distinct() %>%
  mutate(Pred = outcome) %>% dplyr::select(ID, Pred)
#table(submission_fake$outcome, submission_fake$is_known)
submission_fake %>% distinct()
