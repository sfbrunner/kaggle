...
```{r}
train_dat = all_games %>%
  left_join(team_summary, by=c('team1'='team_season'), suffix=c("", "_1")) %>%
  left_join(team_summary, by=c('team2'='team_season'), suffix=c("", "_2"))
train_dat
```
...
```{r}
pipe_logreg = simple_mlr_pipeline(alldat = train_dat %>% dplyr::select(-ID, -set, -team1, -team2, -seed_reg, -seed_reg_2) %>% mutate(Pred = factor(Pred)), 
                            target_feat = 'Pred', 
...
This performs equally well as glmnet. More features should probably be included.
```{r}
pipe_xbg = simple_mlr_pipeline(alldat = train_dat %>% dplyr::select(-ID, -set, -team1, -team2, -seed_reg, -seed_reg_2) %>% mutate(Pred = factor(Pred)), 
                            target_feat = 'Pred', 
...
write.csv(submission_xgb, 'submissions/xgb.csv', quote = F, row.names = F)
```